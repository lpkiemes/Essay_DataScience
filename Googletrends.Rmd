...
for (k in keywords) {
  for (t in times) {
    for (g in geos) {
      ...
      i <- i + 1 # Indexzaehler
      print(paste(i, t, g)) # Iteration anzeigen
      ...
      list_google_data[[i]][["interest_over_time"]] <- list_google_data[[i]][["interest_over_time"]] %>% 
        mutate(hits = as.numeric(str_remove_all(hits, "<"))) # Hits als numerische ohne Kleinerzeichen
...
names(list_google_data) <- names_data # Namen fuer die Daten
```
...
for (n in 1:length(list_google_data)) {
  ...
  ggsave(filename = paste0("plot_line_", names_data[[n]], ".png"), 
...
list_google_plots_maps[[e]] <- ggplot() +
  ...
  ggsave(filename = paste0("plot_map_", names(times)[[e]], ".png"), 
...
list_google_plots_sep[[l]] <- ggplot() +
  ...
  ggsave(filename = paste0("plot_map_2021_", names(colors_locuszoom_light)[l], ".png"), 
