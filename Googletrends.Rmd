...
data_2017_all <- list_data[["prog_5y_int"]][["interest_by_country"]] %>%
  filter(location %in% world$region, hits > 0) %>%
  mutate(region = location, 
         hits = as.numeric(str_remove_all(hits, "%"))) %>%
  select(hits, keyword, region) %>%
...
data_2021_all <- list_data[["prog_12m_int"]][["interest_by_country"]] %>%
  filter(location %in% world$region, hits > 0) %>%
  mutate(region = location, 
         hits = as.numeric(str_remove_all(hits, "%"))) %>%
  select(hits, keyword, region) %>%
...
ggsave("map_2004_all.png", path = here::here("plots"), width = 9, height = 5)
...
ggplot() +
  geom_map(data = world,
           ...
           aes(x = long, y = lat, map_id = region),
           fill="#e5e5e5", color="#bababa", size=0.15) +
  geom_map(data = data_2017_all, 
           map = world, 
           aes(fill = keyword, map_id = region, alpha = hits)) +
  ggtitle("Suchanfragen zu Programmiersprachen weltweit seit Juli 2017*") +
  theme_void() +
  labs(fill = "Suchwort",
       alpha = "Prozentzahl der 
       Suchanfragen",
...
ggsave("map_2017_all.png", path = here::here("plots"), width = 9, height = 5)
...
ggplot() +
  geom_map(data = world,
           ...
           aes(x = long, y = lat, map_id = region),
           fill="#e5e5e5", color="#bababa", size=0.15) +
  geom_map(data = data_2021_all, 
           map = world, 
           aes(fill = keyword, map_id = region, alpha = hits)) +
  ggtitle("Suchanfragen zu Programmiersprachen weltweit seit Juli 2021*") +
  theme_void() +
  labs(fill = "Suchwort",
       alpha = "Prozentzahl der 
       Suchanfragen",
       ...
       Datenquelle: Google Trends 
          (https://www.google.com/trends, abgefragt am ", date_analysis, ").")) +
  scale_fill_manual(values = c("#5CB85CFF", "#46B8DAFF", "#357EBDFF"))

ggsave("map_2021_all.png", path = here::here("plots"), width = 9, height = 5)
```
